<script lang="ts">
	...
	interface Props {
		place: FavouriteWithOptimizedImages;
		onUnfavouriteClick: (placeId: string) => void;
		unfavouritePending?: boolean;
	}

	let { place, onUnfavouriteClick, unfavouritePending }: Props = $props();

	const placeImage = $derived(place.images[0]);

	function handleUnfavouriteClick(e: Event) {
		e.stopPropagation();
		onUnfavouriteClick(place.id);
	}
</script>
		...
		<div class="relative w-full overflow-hidden rounded-lg">
			<div class="group basis-[280px] md:basis-[320px]">
				<div class="relative aspect-[4/3] transition-transform duration-200 group-hover:scale-105">
					<picture>
						<source type="image/webp" srcset={placeImage.webp.srcset} sizes={placeImage.sizes} />
						<img
							src={placeImage.src}
							srcset={placeImage.srcset}
							sizes={placeImage.sizes}
							alt={place.name}
							loading="lazy"
							decoding="async"
					...
					<div class="absolute right-2 top-2 z-10">
						<Button
							...
							onclick={(e) => {
								e.preventDefault();
								e.stopPropagation();
								handleUnfavouriteClick(e);
							}}
						>
							{#if unfavouritePending}
