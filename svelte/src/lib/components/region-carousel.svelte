<script lang="ts">
	...
	import type { Region } from '$lib/types/models';
	...
	let { heading, regions }: Props = $props();
</script>
		...
		<Carousel.Content class="-ml-2 md:-ml-4">
			{#each regions as region}
				<Carousel.Item class="basis-[280px] pl-2 md:basis-[320px] md:pl-4">
					<a href={`/region/${region.slug}`} class="no-underline">
						<Card.Root class="border-0 shadow-none">
							<div class="group relative cursor-pointer">
								<div class="relative aspect-[4/3] overflow-hidden rounded-xl">
									<img
										src={region.optimizedImage.src}
										alt={region.name}
										srcset={region.optimizedImage.srcset}
										sizes={region.optimizedImage.sizes}
										loading="lazy"
